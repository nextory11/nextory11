...
- Authority decision: Director HIRO Decision 001 and Decision 002 — Full Preservation Policy
- Repository role: the single authoritative preservation location for this package
- Current package state: **TECHNICALLY COMPLETE — DIRECTOR PACKAGE ACCEPTANCE PENDING**
...
Expected Question ID ranges are recorded in the Manifest. Luminary is represented by the display type `Luminary`; its Approved JSON currently uses `type: "luminary"`, while the established Question IDs use the `n11-light-bringer-*` slug. This relationship must be preserved unless Director HIRO authorizes a change.
